...
fn two_servers() {
    ...
    let key = include_bytes!("../test/key2.der");
    let key = p!(PrivateKey::from_der(key));
    let cert = include_bytes!("../test/cert2.der");
    let cert = p!(Certificate::from_der(cert));
    let builder = p!(TlsAcceptor::builder2(key, cert, vec![]));
    let builder = p!(builder.build());
    ...
    let root_ca = include_bytes!("../test/cert2.der");
    let root_ca = Certificate::from_der(root_ca).unwrap();
